 ...
 */
class YmlFileImport extends Model
{
    /**
     * @var bool
     */
    ...
    public static function dropTables()
    {
        ...
        $table_name = YmlFile::tableName();

        if ($connection->createCommand("SHOW TABLES LIKE 'yml_import_file'")->query()->count()) {
            $connection->createCommand()->dropTable($table_name)->query();
        }
    }
    ...
    public function _end_element($xmlChunk)
    {
        ...
        if ($child["R"] != $child["RO"]) {
            Yii::$app->db->createCommand(
                'UPDATE ' . YmlFile::tableName() . ' SET rgt = ' . intval($child["R"]) . ' WHERE id = ' . intval($child["ID"]),
                [
