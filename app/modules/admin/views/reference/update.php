...
$this->title = $reference->name.': Редактирование';

$additionalTypes = Json::encode(ReferenceProp::getAdditionalTypes());
// start collect propertyTab
$propertyTabStr = '
    <div class="row">
...
//collect lines for current properties
foreach ($arProperty as $uid => $property) {
    ...
    <div class="form-group<?= ($property->isAttributeRequired('multiple')) ? ' required' : ''; ?>">
        <?= Html::hiddenInput('property[' . $uid . '][multiple]', 0);?>
    ...
    <div class="form-group<?= ($property->isAttributeRequired('active')) ? ' required' : ''; ?>">
        <?= Html::hiddenInput('property[' . $uid . '][active]', 0);?>
...
$propertyTabStr .= '</div>';
$propertyTabStr .= '<p class="text-center" id="add-prop-box">';
$propertyTabStr .= Html::a('<i class="fa fa-plus"></i> Добавить свойство', '#', ['id' => 'new-prop-add', 'class' => 'btn btn-info']);
$propertyTabStr .= '</p>';
?>
<div class="reference-update">
    ...
    <div class="form-group">
        ...
        <?= Html::a('Отмена', ['/admin/reference/index', 'type' => $reference->reference_type_id], ['class' => 'btn btn-default']);?>
    </div>
